...
let cachedOrders: Array<Record<string, unknown>> = [];

declare const bootstrap: any;

let stockModalInstance: any | null = null;
let stockFormInitialized = false;
let currentStockProductId: number | null = null;
let editUserModalInstance: any | null = null;
let currentEditingUserId: number | null = null;
let orderStatusModalInstance: any | null = null;
let orderStatusFormInitialized = false;
let currentOrderStatusOrderId: string | null = null;
...
function showDashboardToast(message: string, variant: ToastVariant = 'info') {
...
function setupProductFormHandlers() {
    const editProductForm = document.getElementById('editProductForm') as HTMLFormElement | null;
...
function setupStockModalHandlers() {
    const stockForm = document.getElementById('productStockForm') as HTMLFormElement | null;
...
function setupOrderStatusFormHandlers() {
    const form = document.getElementById('orderStatusForm') as HTMLFormElement | null;
...
function handleEditProductForm(event: Event) {
    event.preventDefault();

    const idInput = document.getElementById('editProductId') as HTMLInputElement | null;
    if (!idInput) {
    ...
    const nombre = (document.getElementById('editProductName') as HTMLInputElement)?.value.trim() ?? producto.nombre;
    const descripcion = (document.getElementById('editProductDescription') as HTMLTextAreaElement)?.value.trim() ?? producto.descripcion;
    const categoria = (document.getElementById('editProductCategory') as HTMLSelectElement)?.value ?? producto.categoria;
    const peso = (document.getElementById('editProductWeight') as HTMLInputElement)?.value || producto.peso;
    const estadoSeleccionado = (document.getElementById('editProductStatus') as HTMLSelectElement)?.value ?? producto.estado ?? Estado.activo;
    const precio = Number.parseFloat((document.getElementById('editProductPrice') as HTMLInputElement)?.value ?? String(producto.precio ?? 0));
    const stock = Number.parseInt((document.getElementById('editProductStock') as HTMLInputElement)?.value ?? String(producto.stock ?? 0), 10);
...
function handleStockUpdateSubmit(event: Event) {
    event.preventDefault();
    ...
    const quantityInput = document.getElementById('stockQuantity') as HTMLInputElement | null;
    const productIdInput = document.getElementById('stockProductId') as HTMLInputElement | null;
...
function normalizeEstado(value: unknown, defaultEstado: string = Estado.activo): string {
...
function buildProductUpdatePayload(producto: any, overrides: Record<string, unknown> = {}) {
...
function handleEditUserForm(event: Event) {
    event.preventDefault();
    ...
    const idInput = document.getElementById('editUserId') as HTMLInputElement | null;
    if (!idInput) {
    ...
    const nombre = (document.getElementById('editUserNombre') as HTMLInputElement)?.value.trim();
    const apellido = (document.getElementById('editUserApellido') as HTMLInputElement)?.value.trim();
    const email = (document.getElementById('editUserEmail') as HTMLInputElement)?.value.trim();
    const usuario = (document.getElementById('editUserUsuario') as HTMLInputElement)?.value.trim();
    const telefono = (document.getElementById('editUserTelefono') as HTMLInputElement)?.value.trim();
    const rol = (document.getElementById('editUserRol') as HTMLSelectElement)?.value ?? 'cliente';
    const estadoSeleccionado = (document.getElementById('editUserEstado') as HTMLSelectElement)?.value ?? Estado.activo;
...
function handleOrderStatusSubmit(event: Event) {
    event.preventDefault();
    ...
    const idInput = document.getElementById('orderStatusOrderId') as HTMLInputElement | null;
    const select = document.getElementById('orderStatusSelect') as HTMLSelectElement | null;
...
function renderOrdersTable(orders: Array<Record<string, any>>) {
...
function viewUser(userId) {
    const usuario = userManager.obtenerTodosLosUsuarios().find((u: any) => u.id === userId);
    if (!usuario) {
...
function viewOrder(orderId) {
    const manager = window.orderManager;
    const order = cachedOrders.find((o: any) => String(o.id) === String(orderId));
    if (!order) {
...
function editProduct(productId) {
    ...
    (document.getElementById('editProductId') as HTMLInputElement).value = String(producto.id);
    (document.getElementById('editProductName') as HTMLInputElement).value = producto.nombre ?? '';
    (document.getElementById('editProductPrice') as HTMLInputElement).value = String(producto.precio ?? 0);
    (document.getElementById('editProductStock') as HTMLInputElement).value = String(producto.stock ?? 0);
    (document.getElementById('editProductCategory') as HTMLSelectElement).value = producto.categoria ?? '';
    (document.getElementById('editProductDescription') as HTMLTextAreaElement).value = producto.descripcion ?? '';
...
function editUser(userId) {
    ...
    (document.getElementById('editUserId') as HTMLInputElement).value = String(usuario.id ?? '');
    (document.getElementById('editUserNombre') as HTMLInputElement).value = usuario.nombre ?? '';
    (document.getElementById('editUserApellido') as HTMLInputElement).value = usuario.apellido ?? '';
    (document.getElementById('editUserEmail') as HTMLInputElement).value = usuario.email ?? '';
    (document.getElementById('editUserUsuario') as HTMLInputElement).value = usuario.usuario ?? '';
    (document.getElementById('editUserTelefono') as HTMLInputElement).value = usuario.telefono ?? '';
    (document.getElementById('editUserRol') as HTMLSelectElement).value = usuario.rol ?? 'cliente';
    (document.getElementById('editUserEstado') as HTMLSelectElement).value = (usuario.estado ?? usuario.isActivo ?? Estado.activo).toString().toLowerCase();
...
function activateOrder(orderId) {
    const manager = window.orderManager;
    if (!manager?.updateOrderStatus) return;
...
function replenishStock(productId: number) {
    setupStockModalHandlers();
    const manager = window.productManager;
    if (!manager?.obtenerProductoPorId) {
        showDashboardToast('Gestor de productos no disponible', 'error');
    ...
    (document.getElementById('stockProductId') as HTMLInputElement).value = String(producto.id);
...
function editOrderStatus(orderId: string) {
    const manager = window.orderManager;
    if (!manager?.updateOrderStatus) {
        showDashboardToast('Gestor de pedidos no disponible', 'error');
        return;
    ...
    (document.getElementById('orderStatusOrderId') as HTMLInputElement).value = String(orderId);
...
function filterOrders() {
    const filter = (document.getElementById('orderStatusFilter') as HTMLSelectElement)?.value ?? '';
    if (!filter) {
        renderOrdersTable(cachedOrders);
        return;
    }

    const filtrados = cachedOrders.filter((order: any) => String(order.status ?? '').toLowerCase() === filter.toLowerCase());
