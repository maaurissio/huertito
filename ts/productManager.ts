import { CategoriaProducto, Estado, IDataProductos, IProducto } from './models.js';
import { obtenerDatosProductos, guardarDatosProductos } from './servicioDatos.js';

export interface ResultadoValidacionProducto {
  valido: boolean;
  errores: string[];
}

export interface ResultadoOperacionProducto {
  success: boolean;
  mensaje: string;
  producto?: IProducto;
}
...
export class ProductManager {
  ...
  private cargarDatosInternos(): IDataProductos {
  ...
  public cargarDatos(): IDataProductos {
    return this.cargarDatosInternos();
  }

  public obtenerProductosActivos(): IProducto[] {
    const data = this.cargarDatosInternos();
    return data.productos.filter((producto) => (producto.estado ?? producto.isActivo) === Estado.activo);
  }

  public obtenerDatosCompletos(): IDataProductos {
    return this.cargarDatosInternos();
  }

  public obtenerProductosPorCategoria(categoria: string): IProducto[] {
  ...
  public obtenerProductoPorId(id: number | string): IProducto | undefined {
    const idBuscado = typeof id === 'string' ? Number.parseInt(id, 10) : id;
    const data = this.cargarDatosInternos();
    return data.productos.find((producto) => producto.id === idBuscado);
  }

  public obtenerTodosLosProductos(): IProducto[] {
    const data = this.cargarDatosInternos();
    return data.productos;
  }

  public agregarProducto(datosProducto: Partial<IProducto>): ResultadoOperacionProducto {
    try {
      const data = this.cargarDatosInternos();
      ...
      const nuevoProducto: IProducto = {
        ...
        codigo:
          (datosProducto.nombre ?? 'PRD').substring(0, 3).toUpperCase() + data.configuracion.proximoId,
  ...
  public actualizarProducto(productoId: number | string, datosActualizados: Partial<IProducto>): ResultadoOperacionProducto {
  ...
  public verificarYDesactivarSinStock(): ResultadoOperacionProducto {
    try {
      const data = this.cargarDatosInternos();
  ...
  public desactivarProducto(productoId: number | string): ResultadoOperacionProducto {
    const idBuscado = typeof productoId === 'string' ? Number.parseInt(productoId, 10) : productoId;
    try {
      const data = this.cargarDatosInternos();
  ...
  public activarProducto(productoId: number | string): ResultadoOperacionProducto {
    const idBuscado = typeof productoId === 'string' ? Number.parseInt(productoId, 10) : productoId;
    try {
      const data = this.cargarDatosInternos();
      const producto = data.productos.find((p) => p.id === idBuscado);
  ...
  public validarDatosProducto(datos: Partial<IProducto>): ResultadoValidacionProducto {
    ...
    if (!datos.categoria || !Object.values(CategoriaProducto).includes(datos.categoria as CategoriaProducto)) {
...
export function obtenerProductosCatalogo(categoria: string = 'todos'): IProducto[] {
...
declare global {
  interface Window {
    ...
    obtenerProductosCatalogo: typeof obtenerProductosCatalogo;
  }
}
